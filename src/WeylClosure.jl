...
function saturation(g_ :: Vector{OrePoly{T,M}}, i :: Int, A :: OreAlg; bnd::Integer = 1,morestep::Integer=0,stophol::Bool=false,f4b::Bool=false) where {T,M}
...
struct WeylClosureInit{T,C,M,T2,C2,M2}
    SLI :: SingLocInit{T,C,M}
    A :: OreAlg{T2,C2,M2}
end
...
Initialisation function for the Weyl closure
"""
function weyl_closure_init(A :: OreAlg)
    return WeylClosureInit(singular_locus_init(A),add_loc_var("_T","1",A))
end

function weyl_closure_internal(gens::Vector{OrePoly{T,M}}, A :: OreAlg, init :: WeylClosureInit) where {T,M}
    nA = init.A
    ...
    ngens = map_algebras(gens,A,nA)
    gb = saturation(ngens,nA.nrdv+2*nA.npdv +nA.npv+nA.nlv,nA)
...
Returns a Gröbner basis of a subset of the Weyl closure of the module generated by gens.
"""
function weyl_closure(gens::Vector{OrePoly{T,M}}, A :: OreAlg, init :: WeylClosureInit) where {T,M}
    if char(A) > 0 
        return weyl_closure_internal(gens,A, init)
    else 
        return compute_with_CRT(weyl_closure_internal,A, gens, A, init)
    end
end
