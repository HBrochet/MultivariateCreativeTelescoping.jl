...
@testset "dfinite_expr_to_ann reports unclosed parenthesis" begin
    ...
    @test err.value.msg == "Invalid expression: a parenthesis is not closed"
end
...
@testset "dfinite algebraic leaf from minimal polynomial" begin
    ...
    @test gens[1] == parse_OrePoly("dx - 1//(2*x + 2)", A)
end

@testset "dfinite algebraic leaf with non-monic minimal polynomial" begin
    A = OreAlg(order = "lex dx", ratdiffvars = (["x"], ["dx"]))
    gens = dfinite_expr_to_ann(:((x + 1)^(1//3)), A)
    @test length(gens) == 1
end
...
@testset "dfinite algebraic leaf in 2 variables" begin
    ...
    @test length(gens) == 2
    @test gens[1] == parse_OrePoly("dx - 1//(2*x + 2*y + 2)", A)
    @test gens[2] == parse_OrePoly("dy - 1//(2*x + 2*y + 2)", A)
end
...
@testset "dfinite algebraic leaf with nested square roots" begin
    ...
    @test length(gens) == 2
    @test gens[1] == parse_OrePoly("x//(2*y + 2)*dx + dy - 1//(4*y + 4)", A)
    @test gens[2] == parse_OrePoly("dx^2 + x//(x^2 - y - 1)*dx - 1//(4*x^2 - 4*y - 4)", A)
...
@testset "dfinite product with composed coefficients" begin
    ...
    @test gens[1] == parse_OrePoly("dy^2 + ((-4*x*y - 4*y^2 + 1)//(x + y))*dy + ((4*x^2*y^2 - x^2 + 8*x*y^3 - 4*x*y + 4*y^4 - 3*y^2 - 4)//(x^2 + 2*x*y + y^2))", A)
    @test gens[2] == parse_OrePoly("dx^2 + ((-4*x^2 - 4*x*y + 1)//(x + y))*dx + ((4*x^4 + 8*x^3*y + 4*x^2*y^2 - 3*x^2 - 4*x*y - y^2 - 4)//(x^2 + 2*x*y + y^2))", A)
end
...
@testset "dfinite polynomial power with mixed symbolic exponent reports explicit error" begin
    ...
    @test err.value.msg == "Polynomial powers currently support only bare ratvar exponents like s, not mixed expressions like s + 1 / 2"
end

@testset "dfinite product uses polynomial power factor" begin
    A = dfinite_ore_alg(["x"]; ratvars = ["s"])
    gens = dfinite_expr_to_ann("sin(x) * (x + 1)^s", A)
    @test gens[1] == parse_OrePoly("dx^2 - 2*s//(x + 1)*dx + (s^2 + s)//(x^2 + 2*x + 1) + 1", A)
end
